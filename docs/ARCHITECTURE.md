...
| **M3.95 运维闭环** | `tape version`(version+commit+build date+go+platform+install 检测+--json)、`tape update --check/--channel beta/--dry-run`(GitHub Releases API → exec npm/go install,从不后台 ping)、`tape export --split-by none\|size\|agent\|month [--split-size]`(KeepIDs 白名单 + chunk 后缀,session 永不跨 chunk)、`tape sync --install/--uninstall/--status [--interval 1h]`(systemd user timer / LaunchAgent / Windows schtasks 打印,user-scope) | 用户单条命令查清自己跑的版本、一条命令升级、大归档不再一个文件、自动同步零脚手架 | ✅ `internal/schedule` 单测 + `internal/cli` parseSize/parseSplit/planChunks*/insertSuffix/upgradeCommand/detectInstall 单测 + e2e `TestVersion*`/`TestUpdateCheck*`/`TestExportSplit*`/`TestSyncStatus*` 全覆盖 |
