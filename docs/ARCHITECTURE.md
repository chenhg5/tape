...
| **M3.95 运维闭环** | `tape version`(version+commit+build date+go+platform+install 检测+--json)、`tape update --check/--channel beta/--dry-run`(GitHub Releases API → exec npm/go install,从不后台 ping)、`tape export --split-by none\|size\|agent\|month [--split-size] [--jobs N]`(KeepIDs 白名单 + chunk 后缀,session 永不跨 chunk;errgroup 并发 + 平均分 zstd 线程预算)、`ls`/`search`/`export` 加 `--exclude-agent`/`--exclude-dir`/`--exclude-host`(可重复或逗号列表,host 的 `local` 哨兵亦适用)、`tape sync --install/--uninstall/--status [--interval 1h]`(systemd user timer / LaunchAgent / Windows schtasks 打印,user-scope) | 用户单条命令查清自己跑的版本、一条命令升级、大归档分块且能并行写、12 agent 里"全部除某个"一行搞定、自动同步零脚手架 | ✅ `internal/schedule` 单测 + `internal/cli` parseSize/parseSplit/planChunks*/insertSuffix/upgradeCommand/detectInstall/splitCSVAndTrim/resolveExcludeAgents/jobsToEncoderConcurrency 单测 + `internal/archive/local` TestListExcludeFilters + `internal/index/sqlitefts` TestSearchExcludeHost + e2e `TestVersion*`/`TestUpdateCheck*`/`TestExportSplit*`/`TestExportExclude*`/`TestExportJobs*`/`TestSyncStatus*` 全覆盖 |
...
- **npm 分发**(`npm/` + `scripts/release-npm.sh`):esbuild 式平台分包——主包 `@tapeai/tape`(JS launcher,透传语义退出码)通过 `optionalDependencies` 按平台拉取预编译二进制包 `@tapeai/tape-<os>-<arch>`(5 平台,`os`/`cpu` 字段约束),无 postinstall 下载。`release-npm.sh <version> [latest|beta]` 一键构建+发布,dist-tag 区分正式/beta(`npm i -g @tapeai/tape@beta`);包名经 `NPM_PACKAGE` 可改(scoped 包默认带 `--access public`,无 scope 的名字如 `tape-cli` 也支持)。
- **Export 分块**(`tape export --split-by none|size|agent|month`):为应对单机 GB 级归档,`internal/cli/export.go` 在 `runExportChunked` 里把 archive.List 拿到的 summaries 按策略 group(`agent`→ Summary.Agent;`month`→ UpdatedAt 的 `YYYY-MM`;`size`→ 按 `scanSessionSizes` 预扫描的原始字节贪心装桶,默认 256MiB),每 group 调一次 `snapshot.Write` 并通过新加的 `ExportOpts.KeepIDs` 白名单收紧范围(`snapshot.buildKeep` 优先级 KeepIDs > Filter > 全量),输出名按多段扩展名规则插入 chunk 后缀(`tape-export-<ts>.codex.tar.zst` / `.part-001.tar.zst` / `.2026-06.tar.zst`);会话从不跨 chunk 切分(size 模式实际桶大小 = "破阈值前最后一个会话",换可预测性而非精确字节)。
- **Export 并发与 zstd 线程预算**(`--jobs N`):`runExportChunked` 改用 `golang.org/x/sync/errgroup` + `SetLimit(min(jobs, len(chunks)))`,把多个 chunk 写入跑成同时几路;`ExportOpts.EncoderConcurrency` 由 `jobsToEncoderConcurrency(jobs, parallelChunks)` 推导(= `budget / parallelChunks`,至少 1),传给 `zstd.WithEncoderConcurrency`,保证总活跃 goroutine ≈ N 而非 N²,避免 over-subscription 砸到吞吐;`--jobs 0` = `GOMAXPROCS`,`--jobs 1` 给可复现的串行基线;并发模式下 stderr 改成 mutex 保护的 `[i/N] chunk done` 行,不再用 per-file 进度条(并发情况下会闪烁也没意义);JSON envelope 多 `split.jobs` + `split.encoder_concurrency` 让 agent 校验真实并发形态。整块导出虽然只有一路写,也走同一个 EncoderConcurrency 路径——`zstd.NewWriter` 默认就是 `GOMAXPROCS` 并行,`--jobs 1` 就是"我要可复现 benchmark"的开关。
- **反向过滤**(`ls`/`search`/`export` 加 `--exclude-agent`/`--exclude-dir`/`--exclude-host`):`ports.Filter` 与 `ports.Query` 各加 `ExcludeAgents/Projects/Hosts []string`,语义"全部正向 match 后再 AND-NOT 反向",`archive.local.List` 用 `matchAnyAgent/Project/Host` 三个 helper 收尾(对称于 `matchHost`/`matchProject`,host 的 `"local"` 哨兵同样适用——`--exclude-host local` 是"只看远端"的自然写法);`sqlitefts.Search` 在 SQL WHERE 里追加 `AND s.agent <> ?` / `AND NOT (s.project LIKE ? OR instr(s.cwd,?)>0)` / `AND COALESCE(s.host,'') <> ?` 子句,不破坏现有索引选择;CLI 端 `splitCSVAndTrim` 让 `--exclude-agent cc,oc` 与 `--exclude-agent cc --exclude-agent oc` 等价,`resolveExcludeAgents` 复用 `agentid.Normalize` + `Suggest` 给同样的 did-you-mean 体验,去重保证 SQL/loop 不重复。
- **双发行镜像 GitHub+Gitee 自动择优**(`internal/mirror`):每个 release 同时双推到 `chenhg5/tape@github` 与 `cg33/tape@gitee`(`docs/RELEASE.md` 的 release-time 双推 playbook),`internal/mirror.Host` 抽象一个镜像 = `{pingURL, apiLatest, apiList, parser, listParser, userAgent, accept}`,GitHub 与 Gitee 各自一份 parser(GitHub 用 `published_at`、Gitee 用 `created_at` + 拼 `html_url`),`mirror.Pick(ctx)` race 两个 host 的 HEAD probe(`probeTimeout = 1500ms`)取最快,`raceProbe` 所有结果都收(failed probe 用 `probeTimeout*10` 哨兵值)以便 `--debug` 打印每家的延迟,`TAPE_MIRROR=github|gitee|auto` 或 `tape update --mirror <name>` 跳过 probe 硬钉(typo 走 fallback 不 brick);CLI `update` 把 mirror 接到 `updateResult.Mirror` 写进 JSON+history,人类视图加 `via gitee` 后缀;`scripts/install.sh` 用 `curl -X HEAD -w '%{time_total}' --max-time 2` + awk 浮点比较做同等的 Bash 版 probe,asset URL 从 `release_base/download/<tag>/<asset>` 拼出(两边布局一致),CN 用户走 Gitee path 自然加速,npm + go-install 不走 mirror(各自的 proxy 生态已经处理 CDN 加速)。
- **CLI 工艺补完**(参考 tw93/mole 的工艺取舍):`tape completion {bash,zsh,fish,powershell}` 一行 cobra 接出 shell 补全;`-v` / `--debug` 全局 persistent flag → `App.debug` + `debugf(format, args...)` helper,sync 把 source.Detect 的 found/dir/errors 透到 stderr、export 把 chunk plan、update 把 GH URL+timeout 透到 stderr;`internal/oplog` 包以单行 JSONL 落 `~/.tape/operations.log`(`Record{Op,Started,Finished,Duration,Scope,Counts,Bytes,Err,ExitCode}`,append-only + 容忍尾部截断,`TAPE_NO_OPLOG=1` 一键关),`recordedRun` 给 sync/export/restore/update 的 RunE 加个 `defer func(){err = run.finish(err)}()` 就接上,`tape history [--limit N] [--op verb] [--json]` 倒序渲染;`internal/config` 包用 `Defaults{ExcludeAgents/Dirs/Hosts, Jobs, Format, Compress}` 反射 + dot-key(`defaults.jobs`)CRUD,`~/.tape/config.json` 原子写(temp + rename),`tape config {path,get,set,unset,list}` 五子命令,`mergeExcludeAgents/Dirs/Hosts` 让 CLI flag 与 config defaults "合并而非覆盖"(配置是基线偏好、命令行是临时增量),export 在 `Flag.Changed=false` 时拉 `Defaults.Format/Compress/Jobs`;`tape uninstall [--dry-run|--force|--keep-archive]` 走"扫一遍 plan → 用户确认 → 执行"三段式,plan 由 `buildUninstallPlan` 列出 scheduler/archive/oplog/binary 四类 step,binary 永远只打印命令(`npm uninstall -g @tapeai/tape` / `rm $(go env GOBIN)/tape` / `brew uninstall tape`)不自删——bash 用户能注意到、agent 可能不,二者都需要 consent;`scripts/install.sh` 一键拉 GH Releases(`detect OS+arch → 拉 tag → sha256 校验 → install -m 0755 /usr/local/bin/tape`,无 sudo 时回退 `~/.local/bin`),`scripts/setup-raycast.sh` 给 macOS 用户写 5 个 Raycast script-commands。`useJSON()` 早就检测 pipe 自动降级,此项天然具备。
- **版本与自更新**(`tape version` + `tape update`):`internal/cli/version.go` 用 `runtime/debug.ReadBuildInfo` 取 vcs.revision/vcs.time,加 runtime.GOOS/Arch + `detectInstall(path)` 启发式(node_modules→npm / `/opt/homebrew/`、Cellar→homebrew / `$GOBIN`、`$GOPATH/bin`、`~/go/bin`→go-install / 其它→manual),`tape update` 据 install 字段拼实际命令(`npm install -g @tapeai/tape@<tag>` / `go install github.com/chenhg5/tape/cmd/tape@<tag>` / 其它打印手动指引);GitHub Releases API 走 `/releases/latest`(stable)或 `/releases?per_page=5`(`--channel beta`),5s timeout,`User-Agent: tape-cli`;`--check` 报告退出 4 = `errUpdateAvailable`,`--dry-run` 退出 10,真正升级 exec `/bin/sh -c <cmd>`(Windows 走 COMSPEC `/c`);**任何 tape 命令都不会后台 ping GitHub**——隐私默认 opt-out,只有用户执行 update 才有网络出口。
- **Sync 调度**(`tape sync --install/--uninstall/--status [--interval]`):`internal/schedule.Detect()` 按 GOOS 选 backend——Linux `systemd --user`(`~/.config/systemd/user/tape-sync.{service,timer}`,interval ≥ 1m,Install 后 best-effort `systemctl --user daemon-reload && enable --now`,失败打印手动命令)、macOS `launchd` LaunchAgent(`~/Library/LaunchAgents/com.tapeai.sync.plist`,`StartInterval` 秒数,XML 转义 `<>&"'`)、Windows 打印 `schtasks /Create /SC MINUTE` 让用户自跑(任何 elevation/UAC 都比 copy-paste 笨)、其它平台 fallback 打印 `*/N * * * * <cmd>` cron 行并报 error;Job 携带 `os.Executable()` 解析到的 tape 实际路径 + 透传 `--remote` 旗标,所以"在 LaunchAgent 里同步远程机"也成立;全部 user-scope,从不要求 root。
